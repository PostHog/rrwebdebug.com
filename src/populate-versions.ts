...
export function populateVersions(selectedVersion?: string) {
  ...
  Object.entries(versionsJson).forEach(([version, config]) => {
    ...
    groupedSelects[group] += `<option value="${version}" ${isDefault ? "selected" : ""}>${version} (rrweb v${rrwebVersion})</option>`;
  });

  document.getElementById("versions")!.innerHTML = Object.entries(groupedSelects).map(([group, options]) => {
